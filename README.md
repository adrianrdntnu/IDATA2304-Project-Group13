...
### 4. AES Symmetric key cryptography
* Symmetric key cryptography is when the same key is used for both encryption and decryption. The way the data is encrypted using a symmetric key is with the Advanced Encryption Standard (AES), or block ciphers.
...
## Additional Features
### JavaFx Interface
* Provides the user with an interface resembling an administrative application. The user can start and stop a temperature sensor, while getting *live** values visualised on a line chart.
